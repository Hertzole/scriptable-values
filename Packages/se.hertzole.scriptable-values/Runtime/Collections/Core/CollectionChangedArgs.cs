...
namespace Hertzole.ScriptableValues
{
    ...
    public static class CollectionChangedArgsExtensions
    {
        ...
        /// <summary>
        ///     Helper method to get a value from the <see cref="CollectionChangedArgs{T}.NewItems" /> based on the given key.
        /// </summary>
        /// <param name="args">The current args.</param>
        ...
        {
            return TryGetValue(args.NewItems.Span, key, out value);
        }

        /// <summary>
        ///     Helper method to get a value from the <see cref="CollectionChangedArgs{T}.OldItems" /> based on the given key.
        /// </summary>
        /// <param name="args">The current args.</param>
        /// <param name="key">The key to find the value for.</param>
        /// <param name="value">The found value, if any.</param>
        ...
        {
            return TryGetValue(args.OldItems.Span, key, out value);
        }

        private static bool TryGetValue<TKey, TValue>(ReadOnlySpan<KeyValuePair<TKey, TValue>> items, TKey key, out TValue? value)
        {
            if (items.Length == 0)
            {
                value = default;
            ...
            {
                if (equalityComparer.Equals(items[i].Key, key))
                {
                    value = items[i].Value;
                    return true;
                }
