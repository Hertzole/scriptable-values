...
{
    public abstract class ValueListenerTest<TListener, TType, TValue> : BaseRuntimeTest
        where TListener : ScriptableValueListener<TValue> where TType : ScriptableValue<TValue>
    {
        ...
        [Test]
        public void SetValue([ValueSource(nameof(invokeParameters))] InvokeParameters parameters,
            [ValueSource(nameof(valueInvokes))] InvokeEvents invokeOn,
            [ValueSource(nameof(StaticsValue))] TValue value)
        {
        ...
        public void Test_SetValue(InvokeEvents invokeOn, InvokeParameters invokeParameters, TValue initialValue)
        {
            TListener listener = CreateComponent<TListener>();
            TType scriptableValue = CreateInstance<TType>();
            scriptableValue.Value = initialValue;
            TValue toValue = MakeDifferentValue(initialValue);
            listener.TargetValue = scriptableValue;
            ...
            switch (invokeParameters)
            {
                ...
                case InvokeParameters.Multiple:
                    ...
                    {
                        Assert.AreEqual(initialValue, a);
                        Assert.AreEqual(toValue, b);
            ...
            listener.FromValue = initialValue;
            listener.ToValue = toValue;

            scriptableValue.Value = toValue;
