...
public class MyScriptableValue : ScriptableValue<int>
{
    // Called when the value is retrieved using the 'Value' property
    protected override int GetValue()
    {
        return base.GetValue();
    }

    // Called when the value is set using the 'Value' property
    protected override void SetValue(int newValue, bool notify)
    {
        base.SetValue(newValue, notify);
        // You probably don't want to completely override this method, 
        // but you can call the base method and then add your own functionality
    }
}
```

There are more methods you can use and need to be mindfull of when using. See [Creating Custom Types](/guides/creating-custom-types) for more information.
