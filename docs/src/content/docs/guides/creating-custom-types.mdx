...
When exiting play mode, `OnPreDisable` and `OnDisabled` will be called immediately before the editor switches back to edit mode. This means it's safe to unsubscribe from any events directly after exiting play mode.
