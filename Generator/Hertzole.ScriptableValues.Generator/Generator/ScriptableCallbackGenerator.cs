...
public sealed partial class ScriptableCallbackGenerator : IIncrementalGenerator
{
        ...
        in CancellationToken cancellationToken)
    {
        ...
        for (int i = 0; i < context.Attributes.Length; i++)
        {
            ...
            string callbackName = GetCallbackName(context.Attributes[i], in cancellationToken);

            builder.Add((
                hierarchy, new CallbackData(context.TargetSymbol.Name, callbackType, callbackFlags, memberType, scriptableType, genericType!, callbackName)));
        }
    ...
    private static string GetCallbackName(AttributeData attribute, in CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        foreach (KeyValuePair<string, TypedConstant> namedArgument in attribute.NamedArguments)
        {
            ...
            if (namedArgument.Key == "CallbackName" && namedArgument.Value.Value is string name)
            {
                return name;
            }
        }
