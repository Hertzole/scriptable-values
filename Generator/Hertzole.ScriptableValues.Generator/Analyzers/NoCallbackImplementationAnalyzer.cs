        ...
        for (int i = 0; i < wrappers.Length; i++)
        {
            ...
            for (int j = 0; j < attributes.Length; j++)
            {
                if (methodSymbol.Equals(attributes[j].AttributeConstructor, SymbolEqualityComparer.Default))
                {
                    ...
                    if (!attributes[j].NamedArguments.IsDefaultOrEmpty && attributes[j].NamedArguments.Any(kvp => kvp.Key == "CallbackName"))
                    {
                        KeyValuePair<string, TypedConstant> namedArg = attributes[j].NamedArguments.First(kvp => kvp.Key == "CallbackName");
            ...
            callbackName ??= Naming.CreateCallbackName(wrapper.Name, in scriptableType, in flags);
